...
class Website:
    ...
    def saveTXT(self) -> None:
        ...
        Returns:
            None
        """
        with open(f'../output/{self.novel['Title'].replace(' ', '-')}.txt', 'w', encoding="utf-8") as f:
    ...
    def saveEPUB(self) -> None:
        ...
        if self.novel["score"]:
            ...
            book.add_metadata(None, 'meta', '', {'name': 'calibre:rating', 'content': f"{float(normalizedScore)}"})
            book.add_metadata(None, 'meta', '',{'name': 'rating', 'content': f"{float(normalizedScore)}"})
        ...
        if self.novel["Chapters"]:
            book.add_metadata(None, 'meta', '',{'name': 'chapters', 'content': f"{self.novel["Chapters"]}"})
            
        if self.novel["original_language"]:
            book.add_metadata(None, 'meta', '',{'name': 'origLang', 'content': f"{self.novel["original_language"]}"})

        if self.novel["status"]:
            book.add_metadata(None, 'meta', '',{'name': 'status', 'content': f"{self.novel["status"]}"})

        if self.novel["alternate_names"]:
            book.add_metadata(None, 'meta', '',{'name': 'altNames', 'content': f"{self.novel["alternate_names"]}"})
        ...
        for i, chapter in enumerate(self.chapters):
            echapter = epub.EpubHtml(title=f"Chapter {i+1}", file_name=f"chapter-{i+1}.xhtml", lang="en")
